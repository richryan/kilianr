...
For the develping structural model,
...
for (r in 1:1) {
  ...
  # Initial condition
  rpos <- floor(runif(1, min = 0, max = 1) * (tt - pp + 1)) + 1
  print(rpos)
  rY0 <- Y[, rpos, drop = FALSE]
  
  # iid resampling
  ind <- floor(runif(tt - pp, min = 0, max = 1) * (tt - pp)) + 1
  print(ind)
  rU[1:qq, 1:(tt - pp)] <- U[, ind] 
  
  rY[1:(KK * pp), 1] <- NU + A %*% rY0 + rU[1:(KK * pp), 1, drop = FALSE]
  
  for (i in 2:(tt - pp)) {
    rY[1:(KK * pp), i] <- NU + A %*% rY[1:(KK * pp), i - 1, drop = FALSE] + rU[1:(KK * pp), i, drop = FALSE]
  }
  
  ry
}

# pos <- floor(runif(10000, min = 0, max = 1) * (tt - pp + 1)) + 1
# hist(pos)
```
